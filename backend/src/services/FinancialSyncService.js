...
export class FinancialSyncService {
  ...
  static async dashboardSummary(userId, query = {}) {
    ...
    const [accounts, openFinanceTransactions, manualTransactions] = await Promise.all([
      prisma.bankAccount.findMany({ where: { userId } }),
      includeOpenFinance && !paymentMethodFilter
    ...
    const transactions = [...openFinanceItems, ...manualItems].sort((a, b) => b.date - a.date);
    ...
    const manualBalance = includeManual
      ...
      : 0;
    const totalBalance = openFinanceBalance + manualBalance;
